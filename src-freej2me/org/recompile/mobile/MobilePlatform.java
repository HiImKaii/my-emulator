/*
	This file is part of FreeJ2ME.

	FreeJ2ME is free software: you can redistribute it and/or modify
	it under the terms of the GNU General Public License as published by
	the Free Software Foundation, either version 3 of the License, or
	(at your option) any later version.

	FreeJ2ME is distributed in the hope that it will be useful,
	but WITHOUT ANY WARRANTY; without even the implied warranty of
	MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
	GNU General Public License for more details.

	You should have received a copy of the GNU General Public License
	along with FreeJ2ME.  If not, see http://www.gnu.org/licenses/
*/
package org.recompile.mobile;

import java.net.URL;
import java.net.URLClassLoader;
import java.io.InputStream;

import java.awt.event.KeyEvent;

import javax.microedition.lcdui.Display;
import javax.microedition.lcdui.Displayable;
import javax.microedition.lcdui.Canvas;
import javax.microedition.lcdui.game.GameCanvas;
import javax.microedition.lcdui.Image;
import javax.microedition.m3g.Graphics3D;

import java.awt.image.BufferedImage;


/*

	Mobile Platform

*/

public class MobilePlatform
{

	private PlatformImage lcd;
	private PlatformGraphics gc;
	public int lcdWidth;
	public int lcdHeight;

	public MIDletLoader loader;

	public Runnable painter;

	public String dataPath = "";

	public int keyState = 0;

	public MobilePlatform(int width, int height)
	{
		lcdWidth = width;
		lcdHeight = height;

		lcd = new PlatformImage(width, height);
		gc = lcd.getGraphics();

		Mobile.setGraphics3D(new Graphics3D());

		painter = new Runnable()
		{
			public void run()
			{
				// Placeholder //
			}
		};
	}

	public void resizeLCD(int width, int height)
	{
		lcdWidth = width;
		lcdHeight = height;

		lcd = new PlatformImage(width, height);
		gc = lcd.getGraphics();
	}

	public BufferedImage getLCD()
	{
		return lcd.getCanvas();
	}

	public void setPainter(Runnable r)
	{
		painter = r;
	}

	public void keyPressed(int keycode)
	{
		if (Mobile.getDisplay() == null) return;
		updateKeyState(keycode, 1);
		Displayable current = Mobile.getDisplay().getCurrent();
		if (current == null) return;
		if (keycode == -5 || keycode == -7) {
			System.out.println("PLATFORM sending keycode=" + keycode + " to game");
		}
		current.keyPressed(keycode);
	}

	public void keyReleased(int keycode)
	{
		if (Mobile.getDisplay() == null) return;
		updateKeyState(keycode, 0);
		Displayable current = Mobile.getDisplay().getCurrent();
		if (current == null) return;
		current.keyReleased(keycode);
	}

	public void keyRepeated(int keycode)
	{
		if (Mobile.getDisplay() == null) return;
		Displayable current = Mobile.getDisplay().getCurrent();
		if (current == null) return;
		current.keyRepeated(keycode);
	}

	public void pointerDragged(int x, int y)
	{
		if (Mobile.getDisplay() == null) return;
		Displayable current = Mobile.getDisplay().getCurrent();
		if (current == null) return;
		current.pointerDragged(x, y);
	}

	public void pointerPressed(int x, int y)
	{
		if (Mobile.getDisplay() == null) return;
		Displayable current = Mobile.getDisplay().getCurrent();
		if (current == null) return;
		current.pointerPressed(x, y);
	}

	public void pointerReleased(int x, int y)
	{
		if (Mobile.getDisplay() == null) return;
		Displayable current = Mobile.getDisplay().getCurrent();
		if (current == null) return;
		current.pointerReleased(x, y);
	}

	private void updateKeyState(int key, int val)
	{
		int mask=0;
		switch (key)
		{
			case Mobile.KEY_NUM2: mask = GameCanvas.UP_PRESSED; break;
			case Mobile.KEY_NUM4: mask = GameCanvas.LEFT_PRESSED; break;
			case Mobile.KEY_NUM6: mask = GameCanvas.RIGHT_PRESSED; break;
			case Mobile.KEY_NUM8: mask = GameCanvas.DOWN_PRESSED; break;
			case Mobile.KEY_NUM5: mask = GameCanvas.FIRE_PRESSED; break;
			case Mobile.KEY_NUM1: mask = GameCanvas.GAME_A_PRESSED; break;
			case Mobile.KEY_NUM3: mask = GameCanvas.GAME_B_PRESSED; break;
			case Mobile.KEY_NUM7: mask = GameCanvas.GAME_C_PRESSED; break;
			case Mobile.KEY_NUM9: mask = GameCanvas.GAME_D_PRESSED; break;
			case Mobile.NOKIA_UP: mask = GameCanvas.UP_PRESSED; break;
			case Mobile.NOKIA_LEFT: mask = GameCanvas.LEFT_PRESSED; break;
			case Mobile.NOKIA_RIGHT: mask = GameCanvas.RIGHT_PRESSED; break;
			case Mobile.NOKIA_DOWN: mask = GameCanvas.DOWN_PRESSED; break;
			case Mobile.GAME_UP: mask = GameCanvas.UP_PRESSED; break;
			case Mobile.GAME_DOWN: mask = GameCanvas.DOWN_PRESSED; break;
			case Mobile.GAME_LEFT: mask = GameCanvas.LEFT_PRESSED; break;
			case Mobile.GAME_RIGHT: mask = GameCanvas.RIGHT_PRESSED; break;
			case Mobile.GAME_FIRE: mask = GameCanvas.FIRE_PRESSED; break;
		}
		keyState |= mask;
		keyState ^= mask;
		if(val==1) { keyState |= mask; }
	}

/*
	******** Jar Loading ********
*/

	public boolean loadJar(String jarurl)
	{
		try
		{
			// Convert file path to proper URL
			String jarUrlStr = jarurl;
			if (!jarurl.startsWith("file:") && !jarurl.startsWith("http:") && !jarurl.startsWith("https:"))
			{
				jarUrlStr = "file://" + jarurl;
			}
			URL gameJarUrl = new URL(jarUrlStr);

			// Get FreeJ2ME JAR URL (the JAR we're currently running from)
			URL freej2meUrl = getClass().getProtectionDomain().getCodeSource().getLocation();

			// Create URLClassLoader with BOTH FreeJ2ME JAR and game JAR.
			// Class loading strategy (parent-first unless noted):
			//   java.*          → parent (bootstrap/AppClassLoader)
			//   javax.microedition.* → parent (AppClassLoader) — avoids type mismatch
			//   org.recompile.mobile.* → parent (AppClassLoader) — shares static fields
			//   game.* (javak.*, etc.) → child-first via MIDletLoader with ASM instrumentation
			URL[] urls = new URL[]{freej2meUrl, gameJarUrl};
			loader = new MIDletLoader(urls);
			loader.setGameJarUrl(jarUrlStr);

			return true;
		}
		catch (Exception e)
		{
			System.out.println("Error loading JAR: " + e.getClass().getName() + ": " + e.getMessage());
			e.printStackTrace();
			return false;
		}
	}

	public void runJar()
	{
		try
		{
			loader.start();
		}
		catch (Exception e)
		{
			System.out.println("Error Running Jar");
			e.printStackTrace();
		}
	}

/*
	********* Graphics ********
*/

	public void flushGraphics(Image img, int x, int y, int width, int height)
	{
		gc.flushGraphics(img, x, y, width, height);

		painter.run();

		//System.gc();
	}

	public void repaint(Image img, int x, int y, int width, int height)
	{
		gc.flushGraphics(img, x, y, width, height);

		painter.run();

		//System.gc();
	}

}
