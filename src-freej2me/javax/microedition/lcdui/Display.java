...
public class Display
{
	...
	// Inject a character typed on laptop keyboard into the current TextBox/TextField
	public void injectChar(char c)
	{
		if (textInput == null) return;

		String className = textInput.getClass().getName();
		...
		if (c == '\b') {
			...
			if (isTextBox || isTextField) {
				try {
					Class<?> cls = textInput.getClass();
					String text = (String) cls.getMethod("getString").invoke(textInput);
					if (text.length() > 0) {
						cls.getMethod("setString", String.class).invoke(textInput, text.substring(0, text.length() - 1));
					}
				} catch (Exception e) { /* ignore */ }
			}
		} else if (c == '\n' || c == '\r') {
			// Enter — simulate Nokia soft key (ok/select)
			textInput.keyPressed(Mobile.NOKIA_SOFT3);
			textInput.keyReleased(Mobile.NOKIA_SOFT3);
		} else {
			...
			if (isTextBox || isTextField) {
				try {
					Class<?> cls = textInput.getClass();
					String text = (String) cls.getMethod("getString").invoke(textInput);
					int max = (Integer) cls.getMethod("getMaxSize").invoke(textInput);
					if (text.length() < max) {
						cls.getMethod("setString", String.class).invoke(textInput, text + c);
					}
				} catch (Exception e) { /* ignore */ }
			}
		}
		...
		Mobile.getPlatform().repaint(textInput.platformImage, 0, 0, textInput.width, textInput.height);
	}
