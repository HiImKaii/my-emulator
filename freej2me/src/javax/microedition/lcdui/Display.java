...
public class Display
{
	...
	// Current TextBox/TextField for keyboard character injection
	private Displayable textInput;
	...
	public void setCurrent(Displayable next)
	{
		try
		{
			...
			// Update text input mode flag for FreeJ2ME keyboard handling
			String className = current.getClass().getName();
			boolean isText = className.contains("TextBox") || className.contains("TextField");
			org.recompile.freej2me.FreeJ2ME.setTextInputMode(isText);
			textInput = isText ? current : null;
	...
	// Inject a character typed on laptop keyboard into the current Displayable.
	// Works with standard TextBox/TextField AND custom game implementations
	// (e.g. javak framework with static TextField setkey).
	public void injectChar(char c)
	{
		// Prefer textInput (standard TextBox/TextField), fall back to current.
		// Custom game frameworks may not set textInput but still accept character input.
		Object target = (textInput != null) ? textInput : current;
		if (target == null) return;

		String className = target.getClass().getName();
		boolean isTextBox = className.contains("TextBox");
		boolean isTextField = className.contains("TextField");

		if (c == '\b') {
			// Backspace — get string, remove last char, set back
			if (isTextBox || isTextField) {
				try {
					Class<?> cls = target.getClass();
					String text = (String) cls.getMethod("getString").invoke(target);
					if (text.length() > 0) {
						cls.getMethod("setString", String.class).invoke(target, text.substring(0, text.length() - 1));
					}
				} catch (Exception e) { /* ignore */ }
			}
		} else if (c == '\n' || c == '\r') {
			// Enter — simulate Nokia soft key (ok/select)
			try {
				((Displayable)target).keyPressed(Mobile.NOKIA_SOFT3);
				((Displayable)target).keyReleased(Mobile.NOKIA_SOFT3);
			} catch (Exception e) { /* ignore */ }
		} else {
			// Insert character at end — get, append, set back
			if (isTextBox || isTextField) {
				try {
					Class<?> cls = target.getClass();
					String text = (String) cls.getMethod("getString").invoke(target);
					int max = (Integer) cls.getMethod("getMaxSize").invoke(target);
					if (text.length() < max) {
						cls.getMethod("setString", String.class).invoke(target, text + c);
					}
				} catch (Exception e) { /* ignore */ }
			}
		}

		// Repaint after injecting
		Displayable d = (target instanceof Displayable) ? (Displayable) target : current;
		if (d != null) {
			Mobile.getPlatform().repaint(d.platformImage, 0, 0, d.width, d.height);
		}
	}
