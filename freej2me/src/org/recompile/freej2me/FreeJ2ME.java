...
public class FreeJ2ME
{
	...
	private void handlePhoneKey(char c) {
		...
		// Non-text mode: game (e.g. kpah) tracks input by string LENGTH of keyPress count.
		// send (tapCount) keyPresses so the string length equals tapCount.
		// tapCount is 1-indexed position: a=1, b=2, c=3, m=1, n=2, o=3, etc.
		// This correctly produces 'b' when laptop key 'b' is pressed, etc.
		int tapCount = getTapCount(c);
		if (keyCode == 0 || tapCount == 0) return;
	...
	public FreeJ2ME(String args[])
	{
		...
		lcd.addKeyListener(new KeyListener()
		{
			...
			public void keyTyped(KeyEvent e) {
				...
				// Non-text mode: let handlePhoneKey send phone key presses to game.
				// The game (e.g. kpah) tracks text input by counting keyPress events.
				// handlePhoneKey sends each letter as 1 phone keypress (no multi-tap cycling).
				handlePhoneKey(c);
			}
